...
def extraer_convertir():
    ...
    try:
        ...
        for i in range(1, len(bloques), 2):
            ...
            hora = re.search(r'(Hora|Horario)\s*[:\-]?\s*(\d{1,2}:\d{2})(?:\s*hs?)?', texto, re.IGNORECASE)
            causa = re.search(r'(?:Causa\s*:\s*|hecho\s*:\s*|causa\s*)([^\n]+)', texto, re.IGNORECASE)
            ...
            resultado = re.search(r'\s*Resultado\s*+:\s*([^\n]+)', texto, re.IGNORECASE)
            ...
            if re.search(r'\s*det', resultado_texto, re.IGNORECASE):
                detenido_valor = resultado_texto
            else:
                detenido_valor = ""
                
            if re.search(r'\s*cont', resultado_texto, re.IGNORECASE):
                contraventor_valor = resultado_texto
            else:
                contraventor_valor = ""
