...
def log(msg: str):
    ...
    txt_log.text.configure(state="normal")
    txt_log.text.insert("end", msg + "\n")
    txt_log.text.see("end")
...
def extraer_convertir():
    try:
        ...
        for idx, ruta in enumerate(rutas_global, 1):
            ...
            # Nota: fecha se extrae dentro del bloque de datos para el Excel
            re.search(
                r"Fecha\s*:\s*("
        ...
        try:
            ...
            ws = wb.active
            if ws is None:
                log("   ⚠️  No se pudo acceder a la hoja activa")
            ...
            for col in ["G", "H", "K"]:
                for celda in ws[col]:
                    ...
                    if hasattr(celda, "value") and celda.value is not None:
                        t = str(celda.value)
                        for pat in patrones:
                            t = pat.sub("", t)
                        t = re.sub(r"[.\-:;]", " ", t)
            ...
            for row in range(2, ws.max_row + 1):
                ...
                # Verificar que no sea celda combinada y tenga valor válido
            ...
            for row in range(2, ws.max_row + 1):
                ...
                ruta_pdf = celda.value
                if (
                    ruta_pdf
                    and isinstance(ruta_pdf, str)
                    and ruta_pdf.lower().endswith(".pdf")
                ):
                    celda.hyperlink = Hyperlink(ref=f"A{row}", target=str(ruta_pdf))
...
frm_log = ttk.Labelframe(
    root, text=" Registro de procesamiento ", padding=6, bootstyle="secondary"
)
frm_log.pack(fill="both", expand=True, padx=12, pady=(0, 6))

txt_log = ScrolledText(frm_log, height=14, autohide=True, font=("Consolas", 9))
txt_log.pack(fill="both", expand=True)
txt_log.text.configure(state="disabled")  # Deshabilitar edición después de crear
...
lbl_status = ttk.Label(
    frm_status, text="Listo.", bootstyle="secondary", font=("Helvetica", 8)
)
